...
export function connectDossier(doc: Y.Doc, dossierId: string, onReady: () => void): void {
  if (typeof WebSocket === 'undefined') { onReady(); return } // node/jsdom — no transport
  ...
  void Promise.all([import('y-websocket'), import('y-indexeddb')]).then(
    ([{ WebsocketProvider }, { IndexeddbPersistence }]) => {
      ...
      const idb = new IndexeddbPersistence(`dossier:${dossierId}`, doc) as IdbPersistence
      ...
      provider.on('sync', () => { wsReady = true; maybeReady() })
      setTimeout(() => { idbReady = true; wsReady = true; maybeReady() }, 4000)
    },
  )
}
